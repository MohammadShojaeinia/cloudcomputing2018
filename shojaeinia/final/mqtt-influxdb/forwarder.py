    ...
    def _setup_handlers(self):
        ...
        def on_connect(client, userdata, flags, rc):
            ...
            for node_name in self.node_names:
                # topic = "/{node_name}/#".format(node_name="iotdata")
                self.logger.info(
                    "Subscribing to topic %s for node_name %s", "iotdata", "m")
...
def main():
    ...
    parser.add_argument(
        '--mqtt-host', default="192.168.99.100", help='MQTT host')
    parser.add_argument('--mqtt-port', default="1883", help='MQTT port')
    parser.add_argument(
        '--influx-host', default="192.168.99.100", help='InfluxDB host')
    parser.add_argument('--influx-port', default="8086", help='InfluxDB port')
    parser.add_argument('--influx-user', default="admin",
                        help='InfluxDB username')
    parser.add_argument('--influx-db', default="iotdata",
                        help='InfluxDB database')
    parser.add_argument('--node-name', default="m",
    ...
    store = InfluxStore(host=args.influx_host, port=args.influx_port,
                        username=args.influx_user, database=args.influx_db)
